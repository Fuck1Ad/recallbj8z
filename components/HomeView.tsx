...
const HomeView: React.FC<HomeViewProps> = ({ selectedDifficulty, onDifficultyChange, customStats, onCustomStatsChange, onStart, isDark, onToggleTheme, hasSave, onLoadGame }) => {
    const [showChangelog, setShowChangelog] = React.useState(false);
    const [showSponsor, setShowSponsor] = React.useState(false);
    const [showSettings, setShowSettings] = React.useState(false);

    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-100 p-4 md:p-8 flex items-center justify-center transition-colors duration-300">
            {/* Background Decoration */}
             <div className="fixed top-0 left-0 w-full h-full opacity-5 pointer-events-none overflow-hidden z-0">
                 <div className="absolute top-10 left-10 text-[12rem] font-black rotate-12 text-slate-900 dark:text-white">8</div>
                 <div className="absolute bottom-10 right-10 text-[12rem] font-black -rotate-12 text-slate-900 dark:text-white">OI</div>
             </div>
             ...
             <div className="w-full max-w-6xl z-10 grid grid-cols-1 lg:grid-cols-12 gap-4 auto-rows-min">
                 ...
                 <div className="lg:col-span-8 bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-10 shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col justify-between min-h-[400px] relative overflow-hidden group">
                     <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50 dark:bg-indigo-900/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob"></div>
                     <div className="absolute bottom-0 left-0 w-64 h-64 bg-emerald-50 dark:bg-emerald-900/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000"></div>
                     ...
                     <div className="relative z-10">
                         <div className="flex items-center gap-4 mb-2">
                             ...
                             <div>
                                 <h1 className="text-3xl md:text-5xl font-black tracking-tighter text-slate-900 dark:text-white">八中重开模拟器</h1>
                     ...
                     <div className="relative z-10 mt-8">
                             ...
                             <div className="flex flex-wrap gap-2">
                                 {(Object.entries(DIFFICULTY_PRESETS) as [Difficulty, typeof DIFFICULTY_PRESETS['NORMAL']][]).map(([key, config]) => (
                                     <button key={key} onClick={() => onDifficultyChange(key)}
                                         className={`px-4 py-2 rounded-xl border-2 transition-all flex items-center gap-2 font-bold text-sm ${selectedDifficulty === key ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:border-slate-200 dark:hover:border-slate-600'}`}
                                     >
                                 ...
                                 <button onClick={() => onDifficultyChange('CUSTOM')}
                                     className={`px-4 py-2 rounded-xl border-2 transition-all flex items-center gap-2 font-bold text-sm ${selectedDifficulty === 'CUSTOM' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:border-slate-200 dark:hover:border-slate-600'}`}
                                 >
                         ...
                         {selectedDifficulty === 'CUSTOM' && (
                             <div className="mb-6 bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 grid grid-cols-2 gap-x-8 gap-y-2">
                         ...
                         <div className="flex gap-3">
                             <button onClick={onStart} className="flex-1 md:w-auto bg-slate-900 dark:bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black text-xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all flex items-center justify-center gap-3">
                 ...
                 <div className="lg:col-span-4 bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col justify-between h-full relative overflow-hidden">
                     <div className="relative z-10">
                         <h3 className="font-black text-slate-300 uppercase tracking-widest text-xs mb-2">Hall of Fame</h3>
                         <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-4">名人堂</h2>
                         <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-700 p-4 rounded-2xl border border-slate-100 dark:border-slate-600">
                             <div className="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center text-white text-xl shadow-md border-2 border-white dark:border-slate-500">
                                 <i className="fas fa-crown"></i>
                             </div>
                             <div>
                                 <div className="text-xs text-slate-400 font-bold uppercase">All Time Best</div>
                                 <div className="font-black text-slate-800 dark:text-white">虚位以待</div>
                             </div>
                             <div className="ml-auto text-xl font-black text-yellow-500 font-mono">---</div>
                         </div>
                     </div>
                     <div className="absolute -bottom-6 -right-6 text-9xl text-slate-50 dark:text-slate-700 opacity-50 transform rotate-12 z-0"><i className="fas fa-trophy"></i></div>
                 </div>
                 ...
                 <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-5 gap-4">
                     ...
                     <button onClick={() => setShowSponsor(true)} className="bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-md border border-slate-100 dark:border-slate-700 hover:shadow-lg hover:border-amber-100 transition-all flex flex-col items-center justify-center gap-2 group h-full">
                         <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 group-hover:bg-amber-100 group-hover:text-amber-600 transition-colors"><i className="fas fa-heart"></i></div>
                         <span className="font-bold text-slate-600 dark:text-slate-300 text-sm">赞助</span>
                     </button>
             ...
             {showChangelog && (
                 <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-fadeIn" onClick={() => setShowChangelog(false)}>
                     <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-lg w-full shadow-2xl flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                         <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
                             <h2 className="text-2xl font-black text-slate-800 dark:text-white">更新日志</h2>
                             <button onClick={() => setShowChangelog(false)} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors"><i className="fas fa-times"></i></button>
                         </div>
                         <div className="overflow-y-auto custom-scroll space-y-6 pr-2">
                             {CHANGELOG_DATA.map((log, i) => (
                                 <div key={i}>
                                     <div className="flex items-baseline gap-2 mb-2">
                                         <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{log.version}</span>
                                         <span className="text-xs text-slate-400 font-mono">{log.date}</span>
                                     </div>
                                     <ul className="list-disc list-inside space-y-1">
                                         {log.content.map((item, idx) => (
                                             <li key={idx} className="text-sm text-slate-600 dark:text-slate-300">{item}</li>
                                         ))}
                                     </ul>
                                 </div>
                             ))}
             ...
             {showSponsor && (
                 <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-fadeIn" onClick={() => setShowSponsor(false)}>
                     <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                         <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
                             <h2 className="text-2xl font-black text-slate-800 dark:text-white">赞助商展示</h2>
                             <button onClick={() => setShowSponsor(false)} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors"><i className="fas fa-times"></i></button>
                         </div>
                         <div className="overflow-y-auto custom-scroll space-y-4 pr-2 text-center">
                             
                             <div className="p-4 bg-white dark:bg-slate-700 rounded-2xl border border-indigo-100 dark:border-slate-600 shadow-sm mb-2 flex items-center gap-4">
                                <img src="https://pic1.afdiancdn.com/user/dc3ad9282ab911ed94aa52540025c377/avatar/e41d1c836b39710f71c42c3d8c03985b_w3000_h3000_s863.jpeg?imageView2/1/w/240/h/240" 
                                     alt="Anby_" 
                                     className="w-12 h-12 rounded-full border-2 border-indigo-200 dark:border-indigo-800 shadow-sm"
                                />
                                <div className="text-left">
                                    <div className="font-bold text-slate-800 dark:text-white">Anby_</div>
                                    <div className="text-xs text-indigo-500 dark:text-indigo-300 font-bold bg-indigo-50 dark:bg-indigo-900 px-2 py-0.5 rounded-full w-fit">发电榜一</div>
                                </div>
                             </div>

                             <div className="p-4 bg-slate-50 dark:bg-slate-700 rounded-2xl border border-slate-200 dark:border-slate-600 mb-4 opacity-70">
                                <div className="w-12 h-12 bg-slate-200 dark:bg-slate-600 rounded-full mx-auto mb-2 flex items-center justify-center text-xl text-slate-400"><i className="fas fa-plus"></i></div>
             ...
             {showSettings && (
                 <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-fadeIn" onClick={() => setShowSettings(false)}>
                     <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-lg w-full shadow-2xl flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                         <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
                             <h2 className="text-2xl font-black text-slate-800 dark:text-white">设置与关于</h2>
                             <button onClick={() => setShowSettings(false)} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors"><i className="fas fa-times"></i></button>
                         </div>
                         ...
                         <div className="overflow-y-auto custom-scroll space-y-6 pr-2">
                             ...
                             <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700 rounded-2xl">
                                 <div className="flex items-center gap-3">
                                     <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-600 flex items-center justify-center text-slate-600 dark:text-slate-300"><i className={`fas ${isDark ? 'fa-moon' : 'fa-sun'}`}></i></div>
                                     <div>
                                         <div className="font-bold text-slate-800 dark:text-white">深色模式</div>
                                         <div className="text-xs text-slate-400">切换界面颜色主题</div>
                                     </div>
                                 </div>
                                 <button onClick={onToggleTheme} className={`w-12 h-6 rounded-full transition-colors relative ${isDark ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                             ...
                             <div className="space-y-4">
                                 <h3 className="font-black text-slate-400 uppercase tracking-widest text-xs border-b border-slate-100 dark:border-slate-700 pb-2">关于本项目</h3>
            ...
            <div className="fixed bottom-4 left-0 w-full flex justify-center items-center gap-6 text-slate-400 z-0 pointer-events-none md:pointer-events-auto">
                <div className="flex items-center gap-2 text-xs bg-white/50 dark:bg-slate-800/50 backdrop-blur-md px-3 py-1 rounded-full shadow-sm">
